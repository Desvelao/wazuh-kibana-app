...
// It receives a version, revision and/or platform version and updates the package.json file
// Usage: node tags.js --version <version> --revision <revision> --platform-version <platform-version> --manifest-package path/to/package.json
...
async function run(configuration) {
  ...
  for (const platformID in supportedVersions) {
    ...
    logger.info(`Platform: ${platformID} - Base branch: ${branch} - Versions: [${pluginPlatformVersions.join(', ')}] - Manifest plugin path: ${manifestPluginPath}`);
  }
  ...
  for (const platformID in supportedVersions) {
    ...
    for (const pluginPlatformVersion of pluginPlatformVersions) {
      ...
      logger.debug(`Configuration to use with the bump script: ${configuratioBump}`);
